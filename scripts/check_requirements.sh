...
if [ -f /etc/os-release ]; then
    ...
    if [[ "$ID" == "ubuntu" && "$MAJOR_VER" -ge 22 ]]; then
        echo "✅ OS: tested with AAS (version: $PRETTY_NAME)"
    else
        echo "⚠️  OS: not tested with AAS (version: $PRETTY_NAME; recommended: Ubuntu 22.04 or newer)"
    fi
else
    echo "⚠️  OS: unknown (cannot find /etc/os-release)"
fi
...
if command -v nvidia-smi &> /dev/null; then
    ...
    if [ "$DRIVER_MAJOR" -eq 580 ]; then
        ...
        echo "⚠️  NVIDIA Driver: not tested with AAS (version: $DRIVER_VER; recommended: 580; tested on 570, 575)"
    fi
else
...
if docker run --rm hello-world &> /dev/null; then
    ...
    if [ "$DOCKER_MAJOR" -ge 28 ]; then
        ...
        echo "⚠️  Docker Engine: not tested with AAS (version: $DOCKER_VER; recommended: 28 or newer)"
    fi
else
...
if docker info 2>/dev/null | grep -i "runtimes.*nvidia" &> /dev/null; then
    ...
    if [ "$(printf '%s\n' "1.18" "$CTK_VER" | sort -V | head -n1)" = "1.18" ]; then
        echo "✅ NVIDIA CTK: tested with AAS (version: $CTK_VER)"
    else
        echo "⚠️  NVIDIA CTK: not tested with AAS (version: $CTK_VER; recommended: 1.18 or newer)"
    fi
else
